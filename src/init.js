...
export async function initDatabase() {
    ...
    if (!await db.checkOrgs(config.influxdb.org)) {
        console.log(new Date() + ` [INFO] Organization "${config.influxdb.org}" is not found.`);
        console.log(new Date() + ` [INFO] Creating a new organization "${config.influxdb.org}".`);
        db.createOrg(config.influxdb.org);
        await new Promise(resolve => setTimeout(resolve, 2000));
    } else {
        console.log(new Date() + ` [INFO] Organization "${config.influxdb.org}" is found.`);
    }

    // Create bucket if not exists
    if (!await db.checkBucket(config.influxdb.bucket)) {
        console.log(new Date() + ` [INFO] Bucket "${config.influxdb.bucket}" is not found.`);
        ...
        console.log(new Date() + ` [INFO] Bucket "${config.influxdb.bucket}" is found.`);
    }
}